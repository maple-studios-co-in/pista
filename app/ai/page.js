  ...
  return (
    <div className="flex items-center gap-3 rounded-2xl border border-line bg-white p-2.5 shadow-card">
      ...
      <div className="min-w-0 flex-1">
        <Link
          href={`/item/${rec.item.id}`}
          className="block text-[13px] font-semibold leading-tight"
        >
          {rec.item.name}
        </Link>
        <div className="text-[11px] text-muted">
          {formatINR(rec.item.price)} · {rec.item.kcal} kcal
        </div>
      ...
      <button
        onClick={() =>
          add({
            ...rec.item,
            sizes: [{ name: "Regular", price: rec.item.price }],
          })
        }
  ...
  async function ask(text) {
    ...
    try {
      const res = await fetch("/api/ai", {
        ...
        body: JSON.stringify({
          query: q,
          history: messages.slice(-6).map(({ from, text }) => ({ from, text })),
        }),
      });
      const data = await res.json();
      setMessages((m) => [
        ...m,
        { from: "bot", text: data.intro, picks: data.picks },
      ]);
    } catch {
      setMessages((m) => [
        ...m,
        {
          from: "bot",
          text: "Sorry, I couldn't reach the menu just now — try again?",
        },
      ]);
    } finally {
      setBusy(false);
    ...
    <AppShell>
      <div className="bg-gradient-to-b from-brand to-brand-dark px-4 pb-4 pt-4 text-white">
        <div className="flex items-center gap-3">
          <div className="grid h-11 w-11 place-items-center rounded-xl bg-white/15 text-xl">
            ✨
          </div>
          <div>
      ...
      <div className="flex flex-col gap-3 p-4">
        {messages.map((m, i) => (
          <div
            key={i}
            className={`fade-up flex flex-col ${m.from === "me" ? "items-end" : "items-start"}`}
          >
            <div
              className={`max-w-[85%] rounded-2xl px-3.5 py-3 text-[13.5px] leading-relaxed ${
                m.from === "me"
            ...
            >
              {m.lead && (
                <span className="mb-0.5 block font-bold">{m.lead}</span>
              )}
              {m.text}
            </div>
            {m.picks && (
        ...
        {busy && (
          <div className="text-xs text-muted">Shoku AI is thinking…</div>
        )}
        <div ref={endRef} />
      </div>
      ...
      >
        ...
        <button
          type="submit"
          className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-brand text-lg text-white active:scale-90"
        >
          ↑
        </button>
      </form>
